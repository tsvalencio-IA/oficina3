...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(app.t_role === 'admin' || app.t_role === 'gerente') {
        ...
        app.iniciarEscutaFinanceiro();        app.iniciarEscutaValesRH();
    }
    if(app.t_role === 'admin') {
...
app.mostrarTela = function(id, titulo, btn) {
    ...
    const tela = document.getElementById(id); if(tela) tela.style.display = 'block';    const hTitulo = document.getElementById('tituloPagina'); if(hTitulo) hTitulo.innerText = titulo;
...
app.buscarCEP = function(cep) {
    ...
    fetch(`https://viacep.com.br/ws/${cep}/json/`).then(res => res.json()).then(data => {
        if(!data.erro) {
            const elRua = document.getElementById('c_rua');
...
app.abrirModalCRM = function(mode = 'nova', id = '') {
    ...
    if(mode === 'edit') {
        ...
        if(c) {            ['crm_id','c_nome','c_tel','c_doc','c_email','c_cep','c_rua','c_num','c_bairro','c_cidade','c_user','c_pass','c_notas'].forEach(k => {
...
app.apagarCliente = async function(id) {
    if(app.t_role !== 'admin') { app.showToast("Apenas proprietário pode apagar.", "error"); return; }
    if(confirm("Apagar cliente?")) { 
        try {
            const c = app.bancoCrm.find(x => x.id === id);
            await app.db.collection('clientes_base').doc(id).delete();             app.showToast("Cliente Removido.", "success"); 
...
// 4. CHAT
// =====================================================================
app.iniciarEscutaMensagens = function() {    app.db.collection('mensagens').where('tenantId', '==', app.t_id).onSnapshot(snap => {
...
app.enviarAnexoChatGlobal = async function() {
    ...
    try {
        ...
        if(data.secure_url) {
            await app.db.collection('mensagens').add({ tenantId: app.t_id, clienteId: app.chatActiveClienteId, sender: 'admin', text: "📎 Evidência:", fileUrl: data.secure_url, fileType: data.resource_type, lidaCliente: false, timestamp: firebase.firestore.FieldValue.serverTimestamp() });
            inp.value = ''; app.showToast("Enviado!", "success");
        }
    } catch(e) { console.error(e); app.showToast("Falha: " + e.message, "error"); }
};
...
app.iniciarEscutaMensagensInternas = function() {
    app.db.collection('chat_interno').where('tenantId', '==', app.t_id).onSnapshot(snap => {
        ...
        if(area) {
            if(msgs.length === 0) area.innerHTML = '<div class="text-center text-white-50 mt-5 pt-5"><i class="bi bi-headset display-1 opacity-25"></i><p>Chat limpo.</p></div>';
            else {
...
app.salvarEntradaEstoque = async function(e) {
    ...
    try {
        if(id) {
            ...
            if(totalCustoGlobalNF === 0) return app.showToast("Nenhum item válido.", "error");
        ...
        await batch.commit(); app.showToast("Estoque Atualizado!", "success"); e.target.reset(); bootstrap.Modal.getInstance(document.getElementById('modalNF')).hide();
    } catch(err) {
...
app.mudarStatusRapido = async function(id, novoStatus) {
    ...
    try {
        ...
        h.push({  new Date().toISOString(), usuario: app.user_nome, acao: `Moveu para: ${novoStatus.toUpperCase()}` });
...
app.confirmarAtribuicaoBox = async function() {
    ...
    if (checkboxes.length === 0) return app.showToast("Defina 1 mecânico.", "warning");
    let mecanicosArray = []; checkboxes.forEach(c => mecanicosArray.push(c.value)); const mecanicosStr = mecanicosArray.join(' + ');
    
    try {
        const osRef = app.db.collection('ordens_servico').doc(id); const doc = await osRef.get(); let h = doc.data().historico || [];
        h.push({  new Date().toISOString(), usuario: app.user_nome, acao: `Aprovado. [${box}] equipe: [${mecanicosStr}]. Status: BOX` });
        await osRef.update({ status: 'box', mecanicoAtribuido: mecanicosStr, boxAtribuido: box, historico: h, ultimaAtualizacao: new Date().toISOString() });        app.showToast("Para Box!", "success"); bootstrap.Modal.getInstance(document.getElementById('modalAtribuicaoBox')).hide();
    } catch(err) {
...
app.abrirModalNotificacaoWhatsApp = function(id, tipo) {
    ...
    if(os) {
        if(tipo === 'pronto') { document.getElementById('whatsTituloModal').innerText = 'Veículo Pronto!'; document.getElementById('whatsTextoModal').innerText = `Notificar ${os.cliente}?`; }
...
app.dispararWhatsAppAtivo = function() {
    ...
    if (os && os.celular) {
        let texto = ''; if (tipo === 'pronto') texto = `Olá ${os.cliente}! Serviço concluído na *${app.t_nome}*. Aguardamos sua retirada.`;
        window.open(`https://wa.me/55${os.celular.replace(/\D/g, '')}?text=${encodeURIComponent(texto)}`, '_blank');
        const h = os.historico || []; h.push({  new Date().toISOString(), usuario: app.user_nome, acao: `WhatsApp CONCLUSÃO.` });
...
app.abrirModalOS = function(mode = 'nova', id = '') {
    ...
    if (mode === 'edit') {
        ...
        if (os) {
            ...
            if (os.historico) { app.historicoOSAtual = os.historico; app.renderizarHistorico(); }            if (os.pecas) { os.pecas.forEach(p => app.adicionarLinhaPeca(p.desc, p.ncm||'', p.qtd, p.custo, p.venda, p.idEstoque, p.isMaoObra)); }
...
app.salvarOS = async function() {
    try {
        ...
        const novoStatus = document.getElementById('os_status').value;        if(novoStatus === 'box' && id) { const oldData = app.bancoOSCompleto.find(o => o.id === id); if(oldData && oldData.status !== 'box' && !oldData.mecanicoAtribuido) return app.iniciarAtribuicaoBox(id); }

        app.historicoOSAtual.push({  new Date().toISOString(), usuario: app.user_nome, acao: id ? `Editou O.S. Status: ${novoStatus.toUpperCase()}` : "Abriu Ficha." });
        ...
        if (novoStatus === 'entregue') return app.showToast("Use Faturar para baixar estoque.", "warning");
        ...
        app.showToast("Salvo.", "success"); bootstrap.Modal.getInstance(document.getElementById('modalOS')).hide();
    } catch(err) {
...
app.processarFaturamentoCompleto = async function() {
    ...
    try {
        ...
        let h = app.osParaFaturar.historico || []; h.push({  new Date().toISOString(), usuario: app.user_nome, acao: `FATURAMENTO CONCLUÍDO.` });
        ...
        await batch.commit(); app.registrarAuditoriaGlobal("Faturamento", `Faturou ${app.osParaFaturar.placa}.`);
...
app.salvarLancamentoFinanceiro = async function(e) {
    ...
    try {
        ...
        if(id) {
            const m = prompt("JUSTIFICATIVA:"); if(!m || m.trim() === '') return app.showToast("Justificativa obrigatória.", "error");            const sts = document.getElementById('fin_status').value;
            ...
            app.showToast(`Atualizado.`, "success"); app.registrarAuditoriaGlobal("Financeiro", `Editou. Motivo: ${m}`);
        } else {
            ...
            await batch.commit(); app.showToast(`Lançado.`, "success");
        }
...
app.filtrarFinanceiro = function(silent = false) { app.filtroFinDataInicio = document.getElementById('filtroFinInicio').value; app.filtroFinDataFim = document.getElementById('filtroFinFim').value;
...
app.renderizarFinanceiroGeral = function() {
    ...
    document.getElementById('tabelaPagarCorpo').innerHTML = hPagar || '<tr><td colspan="7" class="text-center text-white-50 py-5">Sem dívidas.</td></tr>';
...
app.exportarRelatorioFinanceiro = function() {
    try {
        ...
        doc.setFontSize(12); doc.text(`DRE`, 105, y, { align: "center" }); y += 20;
        ...
        if(bR.length > 0) { doc.autoTable({ startY: y, head: [['Data', 'Referência', 'Modalidade', 'Vlr.', 'Status']], body: bR }); y = doc.lastAutoTable.finalY + 15; }
        ...
        if(bP.length > 0) doc.autoTable({ startY: y, head: [['Data', 'Motivo', 'Modalidade', 'Vlr.', 'Status']], body: bP });
        doc.save(`DRE_${app.t_nome}.pdf`); app.showToast("PDF Exportado", "success");
    } catch(e) { app.showToast("Erro PDF.", "error"); }
};
...
app.salvarFuncionario = async function(e) { e.preventDefault(); try { await app.db.collection('funcionarios').add({ tenantId: app.t_id, nome: document.getElementById('f_nome').value, role: document.getElementById('f_cargo').value, comissao: parseFloat(document.getElementById('f_comissao_mo').value), comissao_pecas: parseFloat(document.getElementById('f_comissao_pecas').value), usuario: document.getElementById('f_user').value, senha: document.getElementById('f_pass').value }); app.showToast("Criado.", "success"); e.target.reset(); bootstrap.Modal.getInstance(document.getElementById('modalEquipe')).hide(); } catch(err) { console.error("Erro:", err); app.showToast("Erro: " + err.message, "error"); } };
app.apagarFuncionario = async function(id) { if(confirm("Bloquear?")) { try { await app.db.collection('funcionarios').doc(id).delete(); app.showToast("Removido.", "success"); } catch(err) { console.error("Erro:", err); app.showToast("Erro: " + err.message, "error"); } } };
...
app.confirmarValeRH = async function(e) {
    e.preventDefault(); try { const idFunc = document.getElementById('vale_id_func').value; const nomeFunc = document.getElementById('vale_nome_func').value; const valor = parseFloat(document.getElementById('vale_valor').value); const motivo = document.getElementById('vale_motivo').value;
    if (valor <= 0) return app.showToast("Valor inválido.", "error");
    const batch = app.db.batch(); const dataH = new Date().toISOString();
    batch.set(app.db.collection('vales_rh').doc(), { tenantId: app.t_id, idFuncionario: idFunc, nomeFuncionario: nomeFunc, valor: valor, motivo: motivo, categoria: 'VALE', dataRealizacao: dataH, responsavel: app.user_nome });    batch.set(app.db.collection('financeiro').doc(), { tenantId: app.t_id, tipo: 'SAIDA', categoria: 'VALE', desc: `[VALE] ${nomeFunc} - ${motivo}`, valor: valor, parcelaAtual: 1, totalParcelas: 1, metodo: 'Dinheiro', vencimento: dataH.split('T')[0], status: 'pago', pessoaId: idFunc, pessoaNome: nomeFunc, origem: 'MANUAL', referenciaId: null, data: dataH });
    await batch.commit(); app.showToast("Vale lançado.", "success"); bootstrap.Modal.getInstance(document.getElementById('modalValeRH')).hide(); } catch(err) { console.error("Erro:", err); app.showToast("Erro: " + err.message, "error"); }
};
...
app.apagarOS = async function() {
    if(app.t_role !== 'admin') return app.showToast("Apenas Admin.", "error");
    const m = prompt("JUSTIFICATIVA:"); if(!m || m.trim() === '') return app.showToast("Abortado.", "error");
    const id = document.getElementById('os_id').value; const osC = app.bancoOSCompleto.find(x => x.id === id);
    if(osC) { try { await app.registrarAuditoriaGlobal(`OS Cancelada: ${osC.placa}`, `Motivo: ${m}`); await app.db.collection('ordens_servico').doc(id).delete(); app.showToast("Lixeira.", "success"); } catch(err) { console.error("Erro:", err); app.showToast("Erro: " + err.message, "error"); } }
...
app.configurarCloudinary = function() {
    ...
    try {
        var w = cloudinary.createUploadWidget({ cloudName: app.CLOUDINARY_CLOUD_NAME, uploadPreset: app.CLOUDINARY_UPLOAD_PRESET, sources: ['local', 'camera'], language: 'pt' }, (err, res) => {
            if (!err && res && res.event === "success") { app.fotosOSAtual.push(res.info.secure_url); app.historicoOSAtual.push({  new Date().toISOString(), usuario: app.user_nome, acao: "Foto." }); app.renderizarGaleria(); }
        });
        document.getElementById("btnUploadCloudinary")?.addEventListener("click", () => w.open(), false);
    } catch(e) { console.error("Erro Cloudinary:", e); }
};
...
app.removerFoto = function(index) { app.fotosOSAtual.splice(index, 1); app.historicoOSAtual.push({  new Date().toISOString(), usuario: app.user_nome, acao: "Excluiu Foto." }); app.renderizarGaleria(); };
...
async function carregarImagemBase64(url) { try { const res = await fetch(url); const blob = await res.blob(); return new Promise(resolve => { const r = new FileReader(); r.onloadend = () => resolve(r.result); r.readAsDataURL(blob); }); } catch(e) { console.error("Erro imagem:", e); return null; } }

app.exportarPDFMenechelli = async function() {
    const btn = document.getElementById('btnGerarPDF'); if(btn) { btn.innerHTML = 'Renderizando...'; btn.disabled = true; }
    ...
    try {
        ...
        doc.setFillColor(30, 41, 59); doc.rect(0, 0, w, 40, 'F'); doc.setTextColor(255, 255, 255); doc.setFont("helvetica", "bold"); doc.setFontSize(26); doc.text(app.t_nome.toUpperCase(), w/2, 22, { align: "center" }); doc.setFontSize(10); doc.setFont("helvetica", "normal"); doc.text(`LAUDO`, w/2, 30, { align: "center" }); y = 50; doc.setTextColor(0, 0, 0);
        doc.setDrawColor(200, 200, 200); doc.rect(15, y, w-30, 25); doc.setFont("helvetica", "bold"); doc.text(`Cliente:`, 20, y+8); doc.setFont("helvetica", "normal"); doc.text(document.getElementById('os_cliente').value, 50, y+8); doc.setFont("helvetica", "bold"); doc.text(`Tel:`, 130, y+8); doc.setFont("helvetica", "normal"); doc.text(document.getElementById('os_celular').value, 150, y+8); doc.setFont("helvetica", "bold"); doc.text(`Placa:`, 20, y+18); doc.setFont("helvetica", "normal"); doc.text(placa, 60, y+18); doc.setFont("helvetica", "bold"); doc.text(`Veículo:`, 130, y+18); doc.setFont("helvetica", "normal"); doc.text(document.getElementById('os_veiculo').value, 148, y+18); y += 35;
        doc.setFont("helvetica", "bold"); doc.setFontSize(12); doc.text(`QUEIXA`, 15, y); doc.line(15, y+2, w-15, y+2); y += 10; doc.setFont("helvetica", "normal"); doc.setFontSize(10); const txtQ = doc.splitTextToSize(document.getElementById('os_relato_cliente').value, w - 30); doc.text(txtQ, 15, y); y += (txtQ.length * 6) + 10;        doc.setFont("helvetica", "bold"); doc.setFontSize(12); doc.text(`DIAGNÓSTICO`, 15, y); doc.line(15, y+2, w-15, y+2); y += 10; doc.setFont("helvetica", "normal"); doc.setFontSize(10); const txtL = doc.splitTextToSize(document.getElementById('os_diagnostico').value, w - 30); doc.text(txtL, 15, y); y += (txtL.length * 6) + 10;
        ...
        if (app.fotosOSAtual.length > 0) { if (y > 220) { doc.addPage(); y = 20; } doc.setFont("helvetica", "bold"); doc.text(`FOTOS`, 15, y); doc.line(15, y+2, w-15, y+2); y += 10; let iX = 15; let iY = y; for (let i = 0; i < app.fotosOSAtual.length; i++) { const b64 = await carregarImagemBase64(app.fotosOSAtual[i]); if(!b64) continue; if (iX + 45 > w - 15) { iX = 15; iY += 53; } if (iY + 45 > 280) { doc.addPage(); iY = 20; iX = 15; } doc.addImage(b64, 'JPEG', iX, iY, 45, 45); iX += 53; } y = iY + 60; }
        doc.save(`OS_${placa}.pdf`); app.showToast("PDF Exportado", "success");
    } catch (e) { console.error("Erro PDF:", e); app.showToast("Erro PDF: " + e.message, "error"); } finally { if(btn) { btn.innerHTML = '<i class="bi bi-file-pdf-fill me-1"></i> Exportar'; btn.disabled = false; } }
};

// =====================================================================
// 12. IA GEMINI - VERSÃO CORRIGIDA E SEGURA
// =====================================================================
    ...
    try {
        ...
        if (doc.exists) {
            ...
            if (app.minhaGeminiKey) {
                console.log("[IA] Chave carregada com sucesso");
            } else {
                console.warn("[IA] Chave não encontrada no Firestore");
            }
            return app.minhaGeminiKey;
        }
    } catch (e) { 
        console.error("[IA] Erro ao buscar chave:", e); 
    }
    return null;
};
    ...
    if(app.t_id) {
        app.db.collection('oficinas').doc(app.t_id).onSnapshot(doc => { 
            if(doc.exists) {
                const data = doc.data();
                app.minhaGeminiKey = data.geminiKey || data.gemini || data.apiGemini || data.api_gemini || data.apiKeyGemini || null;
                if(app.minhaGeminiKey) console.log("[IA] Chave atualizada via listener");
            }
        });
    }
...
app.salvarConhecimentoIA = async function(textoAvulso = null) {
    ...
    try {
        await app.db.collection('conhecimento_ia').add({ tenantId: app.t_id, texto: val, dataImportacao: new Date().toISOString() }); 
        app.showToast("Gravado.", "success"); 
        if(!textoAvulso) document.getElementById('iaConhecimentoTexto').value = '';
    } catch(err) {
...
app.chamarGemini = async function(promptCompleto) {
    if (!app.minhaGeminiKey) {
        console.log("[IA] Buscando chave...");
        await app.carregarGeminiKey();
        if (!app.minhaGeminiKey) {             console.error("[IA] Chave não encontrada"); 
            app.showToast("Erro IA: Chave API não configurada", "error");
    ...
    try {
        ...
        if (!respostaTexto) {
            ...
            app.showToast("Erro IA: Resposta inválida", "error");
...
console.log("🚀 app_oficina.js carregado com sucesso!");